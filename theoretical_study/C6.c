...
void main_J2(int i, int j,int* pioche,int* carte1_J2,int* carte2_J2) 
{
...
double P_approx_2_1_main(int i_prime, int j_prime, int i, int j, int n, int* pioche) 
{
    int count = 0;
    for (int k = 0; k < n; k++)
    {
        int carte1_J2;
        int carte2_J2;
        main_J2(i, j,pioche,&carte1_J2,&carte2_J2);
...
double* P_approx_2_1(int i, int j, int n,int* pioche)
{
    double* t;
    ...
    for(int i_prime=1;i_prime<4;i_prime++)
    {
        for(int j_prime=1;j_prime<4;j_prime++)
        {
            ...
            t[index] = P_approx_2_1_main(i_prime,j_prime,i,j,n,pioche);
            index++;
        }
    }
    return t;
